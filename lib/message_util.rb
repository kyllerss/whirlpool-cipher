...
class MessageUtil
  ...
  def self.pad_byte_array(bytes_array)
    ...
    # create padding such that fully padded message is odd multiple of 256 bits (16 bytes)
    padding = Array.new((256/16) - bytes_array.length % (256/16)) {0x00}
    ...
    message_bit_length = bytes_array.length * 16
  ...
  def self.chunk_into_blocks(original_array)
    ...
    raise 'Message must be a multiple of 512 bits.' unless original_array.length % (512/16) == 0

    bytes_array = original_array.clone

    chunks = Array.new
    while bytes_array.length > 0
      chunks << bytes_array.slice!(0, (512/16))
    end
  ...
  def self.create_message_length_component message_length
    ...
    message_length_component = Array.new((256/16) - hex_string_array.length) {0x00}
